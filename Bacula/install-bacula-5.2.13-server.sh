#!/bin/bash
DB_PASSWORD=$(makepasswd --chars 35)
PG_PASSWORD=$(makepasswd --chars 35)
BDMON_PASSWORD=$(makepasswd --chars 35)
BCONSOLE_PASSWORD=$(makepasswd --chars 35)
BFD_PASSWORD=$(makepasswd --chars 35)
BFDMON_PASSWORD=$(makepasswd --chars 35)
BSD_PASSWORD=$(makepasswd --chars 35)
BSDMON_PASSWORD=$(makepasswd --chars 35)
BACULA_IP="127.0.0.1"
...
cd /usr/src
wget -c  http://www.douglas.wiki.br/Downloads/misc/bacula-5.2.13.tar.gz || { echo "${RED}FALHA AO OBTER O BACULA ${CLOSE}"; exit 1; }
...
cat << EOF > /var/www/application/config/config.php
<?php
...
cp -Rfa /etc/apache2/sites-available/default{,.bkp}
