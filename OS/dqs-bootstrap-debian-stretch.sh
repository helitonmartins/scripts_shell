...
#
#---------------------------------------------------------------------------
# Note: This Shell Script set up the initial configuration to Debian stretch
...
# Version 1:
# Data: 19/06/2017
# Description: Set up the initial configuration of Debian GNU/Linux stretch
...
COMPANY="DQS"
# Configure if the banner needs to be set
BANNER="YES"
...
if [ ${TINY} == "YES" ]; then
  INSTALL_PACKETS="vim vim-scripts vim-doc less locate ntpdate sudo git rsync aptitude libpam-systemd openssh-server"
  ...
  TOOLS="tcpstat ifstat dstat procinfo pciutils dmidecode htop nmap tcpdump usbutils strace ltrace hdparm sdparm iotop atop iotop iftop sntop powertop itop kerneltop dos2unix tofrodos zsh xz-utils unrar libjs-jquery arp-scan"
fi
...
#deb http://ftp.br.debian.org/debian-multimedia/ stretch main non-free
#deb-src http://ftp.br.debian.org/debian-multimedia/ stretch main non-free
